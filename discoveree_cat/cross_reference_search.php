...
if($_SESSION['expired_user']==1 && $_COOKIE['expired_user_cross']==1){
...
setcookie('expired_user_cross', 1, $expire,'/');
}
...
?>
<style>
#parts-list{z-index:1000;float:left;list-style:none;margin-top:-1px;margin-left:-1px;padding:0;width:91%;position: absolute;}
...
<ul id="parts-list">
<?
$result=mysql_query("SELECT partno FROM srai_others.discoveree_live_".$_REQUEST['discoveree_cat_id']."_v2 WHERE 1 AND  partno like '".$_REQUEST['keyword']."%' ORDER BY partno LIMIT 10");
while($row=mysql_fetch_array($result)){?>
<li onClick="selectPart('<?=strtoupper($row[0]);?>','<?=$_REQUEST['c'];?>');"><?=str_replace(strtoupper($_REQUEST['keyword']),'<b>'.strtoupper($_REQUEST['keyword']).'</b>',strtoupper($row[0]));?></li>
<?}?>
</ul>
<?
exit;
...
$c=0;
$sql="SELECT * FROM srai_others.cross_reference_search_setting WHERE NOT label='' AND NOT input_type='' AND discoveree_cat_id=".$_REQUEST['discoveree_cat_id']." ORDER BY priority = 0, priority";
...
$live_sql.=" AND partno in('aidk12s65c5','ffsb2065bdn-f085') ";

if($_SESSION['user_name']=="dnp19716")
$live_sql.=" LIMIT 80000,10000 ";
...
<section id="services">
<div class="container text-left" dstyle="width: 100%;">
...
<?}else{?>
              ...
              <center><h1 style="margin:0px;padding:0px;">Cross Reference Search</h1></center>
                 <!-- /.tab-pane -->
...
<div id="search">
 <form method="POST" action="<?if($_SESSION['user_name']=="dnp1976"){?>compare.php<?}else{?>compare.php<?}?>">
<input type="hidden" name="action" value="cross_search" />
<? if($_REQUEST['id']!=1){?>
...
<div class="col-md-4">
<select required name="discoveree_cat_id" id="discoveree_cat_id"  class="form-control" onchange="if(this.value!=''){location.href='cross_reference_search.php?discoveree_cat_id='+this.value;}">
...
<? if($_REQUEST['id']!=1){?>
<div class="col-md-3">
<label>Type Part No*</label>
<br>
 <input type="text" required autocomplete="off" name="crossreferencepartno" class="parts" cnt="1" id="partno1" placeholder="Part No" style="padding:4px;width:100%;" value="" /><div id="suggesstion-box1"></div>
 </div>
...
?>
<div class="col-md-3">
<label><?=$v['label']?> <?if($v['search_perc']>0 || $v['search_perc_plus']>0){echo "(Adjust in %)";}?></label>
<br />
<? if(strpos($v['input_type'],"dropdown")!==false){?>
<select operator="" link_condition="<?=$v['link_value']?>" name="<?=$v['link_with']?><? if(strpos($v['input_type'],"dropdownm")!==false){?>[]<?}?><? if(strpos(strtolower($v['label']),"mounting")!==false){?>_mounting<?}?>" multiplyby="<?=$v['multiplyby']?>"  input_type="<?=$v['input_type']?>"  link_fld="<?=$v['fieldname']?>" str="<?=$v['label']?>" <? if(strpos($v['input_type'],"dropdownm")!==false){?>multiple<?}?> class="form-control <? if(strpos($v['input_type'],"dropdownm")!==false){?>dropdownmultiselect<?}?>" id="d<?=$ccnt;?>" <? if(strpos($v['input_type'],"dropdownm")===false){?>sonchange="datacall();"<?}?>>
...
if(strpos(strtolower($v['label']),"mounting")!==false){
$datavalue=trim(strtoupper(substr($datavalue,0,strpos($datavalue," "))));
}

if(!in_array($datavalue,$founddata) && $datavalue!=""){
?>
<option <? if(strpos($v['input_type'],"dropdownm")!==false){?>SELECTED<?}?> value="<?=($datavalue=="NA"?"":$datavalue);?>"><? if(strpos(strtolower($v['label']),"mounting")!==false){echo $datavalue;}else{echo checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],manflabel($datavalue),$v['value_type']);}?></option>
<?
$founddata[]=$datavalue;
}}}?>
</select>
...
$(document).click(function(){
  $("#suggesstion-box1,#suggesstion-box2,#suggesstion-box3,#suggesstion-box4,#suggesstion-box5").hide();
});
	...
	discoveree_cat_id=<?=$_REQUEST['discoveree_cat_id'];?>;
     ...
     <?}?>
		$.ajax({
		type: "POST",
		url: "cross_reference_search.php",
		data:'discoveree_cat_id='+discoveree_cat_id+'&autosearch=true&c='+key+'&keyword='+$(this).val(),
		beforeSend: function(){
