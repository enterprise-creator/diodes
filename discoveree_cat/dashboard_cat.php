...
if((strpos($_SESSION['opage_access'],"id=".$_REQUEST['id']."&discoveree_cat_id=".$_REQUEST['discoveree_cat_id'])===false && $_REQUEST['discoveree_cat_id']!="") || (strpos($_SESSION['opage_access'],"id=".$_REQUEST['id'])===false && $_REQUEST['discoveree_cat_id']=="")){
...
if($_SESSION['expired_user']==1 && $_COOKIE['expired_user_dashboard_cat']==1){
...
setcookie('expired_user_dashboard_cat', 1, $expire,'/');
}
...
$diode_cond=0;
$dss=mysql_query("SELECT * FROM srai_others.dashboard_category_order WHERE NOT catname LIKE '%zener%' AND  NOT catname LIKE '%tvs%' AND dashboard_setting_id=".$_REQUEST['id']." ORDER BY catorder");
while($drow=mysql_fetch_array($dss)){
...
$catlist=explode(",",$dashboard_setting['catlist']);
...
$eq="SELECT symbol_field,condition_type,value FROM srai_others.discoveree_error_reporting_condition  WHERE NOT condition_type='Null' AND discoveree_cat_id in (".$dashboard_setting['catlist']."0) and symbol_field in ('".$field_x."','".$field_y."') GROUP BY symbol_field,condition_type,value";
...
$manf_search_words=(mysql_query("SELECT title,save_word,no_word FROM srai_others.manf_search_words WHERE discoveree_cat_id in (".$dashboard_setting['catlist']."0) GROUP BY title,save_word,no_word ORDER BY save_word,no_word"));
while($mswrow=mysql_fetch_array($manf_search_words)){
$msw[$mswrow[0]].=trim($mswrow[1])."||".trim($mswrow[2])."||";
}
}

$dresults=mysql_query("select * FROM srai_others.part_split_field_value WHERE discoveree_cat_id in (".$dashboard_setting['catlist']."0)    ORDER BY id ");
...
foreach($qs as $b){
if(strpos($b,'c=')===false && strpos($b,'id=')===false && strpos($b,'cid[]=')===false && strpos($b,'discoveree_cat_id=')===false && strpos($b,'legend_view=')===false && strpos($b,'d1=')===false && strpos($b,'d2=')===false && strpos($b,'d3=')===false && strpos($b,'d4=')===false && strpos($b,'d5=')===false && strpos($b,'d6=')===false && strpos($b,'d7=')===false && strpos($b,'d8=')===false){
...
if(abs($b[1])>0){
$b1=explode("@",$b[0]);
$b1[2]=str_replace("maximum","greatest",$b1[2]);

$paramfld[$b1[0].$b1[3]]=$b[1];

$tmp=$b1[3]=substr($b1[3],0,strlen($b1[3])-1);

if(strpos($b1[3],",")!==false){
$exc=explode(",",$b1[3]);
...
for($iii=0;$iii<count($exc);$iii++){
$ekk=0;
$ncond="";
foreach($exc as $ek=>$ec){
if($exc[$iii+$ek]!=""){
$ncond.=$exc[$iii+$ek].",";
}else{
$ncond.=$exc[$ekk].",";
...
$ncond=substr($ncond,0,strlen($ncond)-1);

if($b1[2]=="greatest")
$scond.="COALESCE(".$ncond."),";
else
$scond.="if(COALESCE(".$ncond.")=0,99999,COALESCE(".$ncond.")),";
}

$scond=substr($scond,0,strlen($scond)-1);
$scond.=")";
}else{
$scond=$b1[3];
}
...
$cnt=0;
foreach($catlist as $discoveree_cat_id){
if($discoveree_cat_id>0){
$dashboard_setting['discoveree_cat_id']=$discoveree_cat_id;
...
$live_sql.=$spc;
//if($_SESSION['test_user_id']>0 && $_REQUEST['id']>0 && $_REQUEST['discoveree_cat_id']==0)
//$live_sql.=" AND manf in ('vishay','diotec') ";

  $live_sql.=" ".$condition." ";
// $live_sql.=" LIMIT 315,5 ";

if(strpos($dashboard_setting['fieldx'],'package')!==false)
$live_sql.=" ORDER BY discoveree_area DESC";
$resultt[]=$tt=mysql_query($live_sql);
$totreccount=$totreccount+mysql_num_rows($tt);
}
}
...
if($bh['value_array']!="" && strpos($bh['discoveree_cat_id'],$dashboard_setting['discoveree_cat_id'])!==false)
...
if($totreccount<=$totrecshow && $totreccount>0){
$_REQUEST['c']=1;
...
$value=$value_array[$key][trim($value)]['org_value'];
    ...
    $value=trim($value);
     if($key=="package" || (strpos($filter_search,$key.",")!==false && $key!="user_id" && $key!="fixingtime_min" && $key!="searchword" && strlen($key)>=2 && substr($key, -2)!="tc" && substr($key, -3)!="tc1" && substr($key, -3)!="tc2"  && substr($key, -3)!="tc3"  && substr($key, -3)!="tc4" && strpos($value,"=")===false && strpos($value,",")===false && strpos($value,";")===false)){
     ...
     $data[$c][$cnt][$key]=str_replace("?","",utf8_decode(str_replace(",","",str_replace("(","",str_replace(")","",str_replace("[","",str_replace("]","",$vvv)))))));
      //if($data[$c][$cnt][$key]=="")
      //$data[$c][$cnt][$key]="0";
     }
     if($key=="searchword"){
...
foreach($link_valuex as $ddk=>$ddv){
 ...
 if(abs($data[$c][$cnt]['y'.$ddk])>0 && $multiplyy[$ddk]>1)
  ...
  unset($data[$c][$cnt]);
  else if((abs($data[$c][$cnt]['y'])==0 && abs($data[$c][$cnt]['x'])==0) && strpos($dashboard_setting['fieldx'],"package")===false)
  unset($data[$c][$cnt]);
$cnt++;
}
}
...
$sql="SELECT * FROM srai_others.product_page_setting WHERE discoveree_cat_id in (".$dashboard_setting['discoveree_cat_id'].") ORDER BY priority";
...
<link rel="stylesheet" href="https://www.discoveree.io/css/protip.min.css">
<link href="https://www.discoveree.io/dist/css/jquery.multiselect.css" rel="stylesheet" type="text/css">
<link rel="stylesheet" href="https://www.discoveree.io/assets/fonts/font-awesome/font-awesome.min.css">
     <style>
	canvas {
...
<div class="nav-tabs-custom">
 ...
 <?if($_REQUEST['discoveree_cat_id']>0){?>
  <input type="hidden" name="discoveree_cat_id" value="<?=$_REQUEST['discoveree_cat_id'];?>" />
 <?}?>
 <?}?>
<div id="search" <?if($totreccount<=$totrecshow && $totreccount>0){?>style="display:none;"<?}?>>
...
if(strpos($_SERVER['QUERY_STRING'],'d1=')!==false)
$getdefaultsearch[0]=urldecode(substr($_SERVER['QUERY_STRING'],strpos($_SERVER['QUERY_STRING'],'d1=')-1,strlen($_SERVER['QUERY_STRING'])));

if(mysql_num_rows($saveresult)>0){?><a id="href_mysaved_search" onclick="$('#savedsearchform').load('../extraactions.php?action=showsaved&pageurl=<?=str_replace("&","@and@",$_SERVER['REQUEST_URI']);?>');jQuery('#modal-mysavedsearch').modal('show');" style="cursor:pointer;">[ My Saved Search <i class="fa fa-list" title="My Saved Search" style="font-size: 12px;"></i> ]</a><?}?> <a id="savesearch_div"  onclick="setsavetext(0);" style="cursor:pointer;display:none;">[ Save This Search <i class="fa fa-save" title="Save" style="font-size: 12px;"></i> ]</a><div style='clear:both;'></div>
<? $cnt=0;
foreach($search as $k=>$v){
?>
<?if((strpos($v['input_type'],"dropdown")!==false && count($data)>0) || strpos($v['label'],"Category")!==false || (strpos($v['input_type'],"dropdown")===false && $totreccount<=$totrecshow && $totreccount>0) || (strpos($v['input_type'],"textboxm")!==false && $diode_cond==0 && strpos($v['label'],"Range")===false && ($totreccount>$totrecshow || $totreccount==0))){
$cnt++;
...
<select <?if(($totreccount>$totrecshow || $totreccount==0) && strpos($v['label'],"Category")!==false){?>required name="cid[]"<?}?> operator="" link_condition="<?=$v['link_value']?>" link_with="<?=$v['link_with']?>" multiplyby="<?=$v['multiplyby']?>"  input_type="<?=$v['input_type']?>"  link_fld="<?=$v['fieldname']?>" str="<?=$v['label']?>" <? if(strpos($v['input_type'],"dropdownm")!==false){?>multiple<?}?> class="form-control <? if(strpos($v['input_type'],"dropdownm")!==false){?>dropdownmultiselect<?}?>" id="d<?=$cnt;?>" <? if(strpos($v['input_type'],"dropdownm")===false){?>onchange=""<?}?>>
<? if(strpos($v['input_type'],"dropdownm")===false){?>
<option SELECTED value="">-Select All-</option>
<?}?>
<?
if(($totreccount>$totrecshow  || $totreccount==0) && strpos($v['label'],"Category")!==false){
foreach($othercat as $o=>$c){
if(count($catl)<=0 || in_array($o,$catl)){
echo "<option value='".$o."'>".checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$c,$v['value_type'])."</option>";
}else{
echo "<option value='".$o."'>".checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$c,$v['value_type'])."</option>";
}
}
...
if(!in_array($datavalue,$founddata[substr($v['fieldname'],0,strlen($v['fieldname'])-1)]) && $datavalue!=""){
...
?>
<option <? if((strpos($v['input_type'],"dropdownm")!==false && $getdefaultsearch[0]=="") || strpos($getdefaultsearch[0],$exdva)!==false){?>SELECTED<?}?> value="<?=$exdva;?>">
<?
if($v['link_with']=="discoveree_cat_id"){
echo checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$othercat[$exdva],$v['value_type']);
}else{
echo str_replace("Non-auto","Non-Auto",checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$exdva,$v['value_type']));
}
?>
</option>
<?
...
if(strpos($datavalue,",")===false){
$dd_val=str_replace("Non-auto","Non-Auto",checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$datavalue,$v['value_type']));
?>
<option <? if((strpos($v['input_type'],"dropdownm")!==false && $getdefaultsearch[0]=="" && ($dfm==0 || $default_manf[$dd_val]==1 || $default_manf_lower[strtolower(str_replace(" ","",$dd_val))]==1 || substr($v['fieldname'],0,strlen($v['fieldname'])-1)!="manf")) || strpos($getdefaultsearch[0],$datavalue)!==false){?>SELECTED<?}?> value="<?=$datavalue;?>">
<?
if($v['link_with']=="discoveree_cat_id"){
echo checksplit(substr($v['fieldname'],0,strlen($v['fieldname'])-1),$_REQUEST['discoveree_cat_id'],$othercat[$datavalue],$v['value_type']);
}else{
echo $dd_val;
}?>
</option>
...
<input value="<?=$paramfld['min'.$v['fieldname']];?>" operator=">=" name="min@<?=$v['multiplyby']?>@<?=$v['link_value']?>@<?=$v['fieldname']?>" input_type="<?=$v['input_type']?>" multiplyby="<?=$v['multiplyby']?>" link_condition="<?=$v['link_value']?>" link_with="<?=$v['link_with']?>"  link_fld="<?=$v['fieldname']?>" type="text" onkeyup="" class="form-control" id="min<?=$cnt;?>" placeholder="Min" style="float:left;width:47%;text-align:center;" /><div style="margin-top:5px;float:left;width:5%;text-align:center;">-</div><input value="<?=$paramfld['max'.$v['fieldname']];?>" onkeyup="" name="max@<?=$v['multiplyby']?>@<?=$v['link_value']?>@<?=$v['fieldname']?>" multiplyby="<?=$v['multiplyby']?>" input_type="<?=$v['input_type']?>" link_condition="<?=$v['link_value']?>" link_with="<?=$v['link_with']?>"  link_fld="<?=$v['fieldname']?>" operator="<=" type="text" class="form-control" id="max<?=$cnt;?>" placeholder="Max" style="float:left;width:47%;text-align:center;" />
<?}?>
...
?>
<?if(($_REQUEST['c']=="" || $totreccount==0) && $diode_cond==1){?>
<div class="col-md-3">
<label>Rated Reverse Voltage, VR (V)</label>
<br />
<input name="min@1@exact@voltage," type="text" onkeyup="" class="form-control" placeholder="Min" style="float:left;width:47%;text-align:center;" /><div style="margin-top:5px;float:left;width:5%;text-align:center;">-</div><input name="max@1@exact@voltage," type="text" class="form-control" placeholder="Max" style="float:left;width:47%;text-align:center;" />
</div>

<div class="col-md-3">
<label>Max. Forward Voltage, VF (V)</label>
<br />
<input name="min@1@least@vfmax1,vfmax2,vfmax3,vfmax4," type="text" onkeyup="" class="form-control" placeholder="Min" style="float:left;width:47%;text-align:center;" /><div style="margin-top:5px;float:left;width:5%;text-align:center;">-</div><input name="max@1@least@vfmax1,vfmax2,vfmax3,vfmax4," type="text" class="form-control" placeholder="Max" style="float:left;width:47%;text-align:center;" />
</div>

<div class="col-md-3">
<label>Power Dissipation, PD (W)</label>
<br />
<input name="min@1@greatest@pdmax1,pdmax2,pdmax3," type="text" onkeyup="" class="form-control" placeholder="Min" style="float:left;width:47%;text-align:center;" /><div style="margin-top:5px;float:left;width:5%;text-align:center;">-</div><input name="max@1@greatest@pdmax1,pdmax2,pdmax3," type="text" class="form-control" placeholder="Max" style="float:left;width:47%;text-align:center;" />
</div>
...
<?if($totreccount<=$totrecshow && $totreccount>0){?>
<a onclick="$('#loading_layer').show();datacall();" class="btn btn-success" style="cursor:pointer;">Search</a>  <a href="dashboard_cat.php?id=<?=$_REQUEST['id'];?>&type=<?=$_REQUEST['type'];?>" class="btn btn-danger">Back To Main Search</a></div>
<?}else{?>
...
<?}else{?>
<center><b>We found a total of <?=$totreccount;?> parts. Please narrow down the selection to avoid long loading time for the dashboard.</b></center>
<?}?>
<?}?>
...
?>
<div id="graph_legend" style="display:none;margin-left:5px;float:left;font-size: 10px;">
 <?=$dashboard_setting['legend_view_label'];?>:

  <select class="form-controls" id="legend_view" style="padding: 2px;" onchange="open_legend(this.value);">
  <? if(strpos($dashboard_setting['legend_view'],'manf')!==false){?>
   ...
   <option value="qualification" <?if($_REQUEST['legend_view']=="qualification"){echo "SELECTED";}?>>Qualification</option>
    <?}?>
   <? if(strpos($dashboard_setting['legend_view'],'configuration11')!==false){?>
   <option value="configuration" <?if($_REQUEST['legend_view']=="configuration"){echo "SELECTED";}?>>Configuration</option>
    <?}?>
   <? if(strpos($dashboard_setting['legend_view'],'package11')!==false){?>
...
<script src="https://www.discoveree.io/admin/Chart.bundle.js"></script>
<script src="https://www.discoveree.io/admin/utils.js"></script>
<script src="https://www.discoveree.io/plugins/jQuery/jquery.min.js"></script>
<script src="https://www.discoveree.io/bootstrap/js/bootstrap.min.js"></script>
<script src="https://www.discoveree.io/dist/js/jquery.multiselect.js"></script>
<?include("../footer.php");?>
<script>
var gs = {};
<?
...
var color = Chart.helpers.color;
	var scatterChartData = {
        ...
        $legend_count=0;
        foreach($data as $k=>$v){
        if($dashboard_setting['legend']!="manf" || ($_REQUEST['legend_view']!="" && $_REQUEST['legend_view']!="manf") || ($dashboard_setting['legend']=="manf" && ($dfm==0 || $default_manf[$k]==1 || $default_manf_lower[strtolower(str_replace(" ","",$k))]==1))){
        $legend_count++;
        $lk=str_replace(" ","-",$k);
        ?>
        {
	...
	$('#search,#toggleScale_x,#toggleScale_y,#graph_legend').show();
	$('#loading').hide();
	total_searches();
...
<script>
 ...
 $(".form-control").each(function(){
  if($(this).val()!="" && $(this).attr('id')!="viewd" && $(this).attr('id')!="legend_view" && $(this).attr('id')!="jitter"){
  tot_searches++;
  }
 });
...
$.each(tempArray, function(key, value) {
  ...
  if($(this).attr('id')!="viewd" && $(this).attr('id')!="legend_view" && $(this).attr('id')!="jitter"){
...
/////////////////save search-end
function open_legend(s){
$('#loading_layer').show();
d=setsavetext(1);
location.href='?id=<?=$_REQUEST['id'];?>&c=1&<?=$qst;?>legend_view='+s+d;
}
//////////////// saved result-start
function save_load(){
...
<script src="https://www.discoveree.io/js/protip.min.js"></script>
<script>
